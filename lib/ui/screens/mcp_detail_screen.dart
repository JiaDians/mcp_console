...
class McpDetailScreen extends ConsumerWidget {
  ...
  void _showUpdateDialog(
    ...
    showDialog(
      ...
      builder: (_) => _UpdateDialog(server: server, newVersion: newVersion),
    );
  }
}
...
class _UpdateDialog extends ConsumerStatefulWidget {
  ...
  const _UpdateDialog({required this.server, required this.newVersion});
...
class _UpdateDialogState extends ConsumerState<_UpdateDialog> {
  ...
  Future<void> _verifyUpdate() async {
    ...
    // Clear cache so local detection picks up the newly installed version
    LocalVersionService.clearCache();
    final updatedServer =
        widget.server.copyWith(currentVersion: widget.newVersion);
    final result = await VersionCheckService().check(updatedServer);
    if (mounted) setState(() { _verifyResult = result; _verifying = false; });
  }
        ...
        if (!_running)
          FilledButton(
            onPressed: () {
              ...
              // Refresh the version section in the detail screen
              LocalVersionService.clearCache();
              ref.invalidate(versionCheckProvider(widget.server));
            },
            child: const Text('關閉'),
          ),
      ],
  ...
  Widget build(BuildContext context) {
    ...
    if (result.updateAvailable) {
      return Row(children: [
        ...
        const SizedBox(width: 6),
        Text(
          '仍未更新 — 最新版為 v${result.latestVersion}',
